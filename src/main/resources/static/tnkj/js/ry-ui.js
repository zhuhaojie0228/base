...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			// 列超出指定长度浮动提示（单击文本复制）,并且点击打开数据详情弹框
			detailTip:function(url,value, length) {
				var _length = $.common.isEmpty(length) ? 20 : length;
				var _text = "";
				var _value = $.common.nullToStr(value);
				if (_value.length > _length) {
					...
					var actions = [];
					actions.push($.common.sprintf('<input id="tooltip-show" style="opacity: 0;position: absolute;z-index:-1" type="text" value="%s"/>', _value));
					actions.push($.common.sprintf("<a onclick='$.table.dataDetail(\""+url+"\")' href='###' class='tooltip-show' data-toggle='tooltip' title='%s'>%s</a>", _value, _text));
					return actions.join('');
				} else {
					_text = _value;
					return _text;
        ...
        operate: {
			...
			// 受理信息
			deal: function(status,id) {
        		if(status==2){
					$.modal.alertWarning("您好，该条" + $.table._option.modalName + "信息已受理，请检查！");
				}else if(status==3){
					$.modal.alertWarning("您好，该条" + $.table._option.modalName + "信息已拒绝，请检查！");
				}else{
					$.modal.confirm("确定受理该条" + $.table._option.modalName + "信息吗？", function() {
						var url = $.common.isEmpty(id) ? $.table._option.dealUrl : $.table._option.dealUrl.replace("{id}", id);
						var data = { "ids": id };
						$.operate.submit(url, "post", "json", data);
					});
				}
			...
			dealAll: function() {
				...
				var statusRow =$.table.selectColumns('status');
				if(statusRow.indexOf("2") >-1 || statusRow.indexOf("3") >-1){
					$.modal.alertWarning("您好，您选中的" + $.table._option.modalName + "信息中包含已受理或者拒绝的数据，请检查！");
				}else{
					$.modal.confirm("确认要受理选中的" + rows.length + "条数据吗?", function() {
						var url = $.table._option.dealUrl;
						var data = { "ids": rows.join() };
						$.operate.submit(url, "post", "json", data);
					});
				}
			...
			refuse: function(status,id) {
				...
				if(status==2){
					...
					$.modal.open("拒绝" + $.table._option.modalName,$.table._option.refuseUrl+'/'+id,800,220);
				}
			},
			// 批量拒绝信息
			/*refuseAll: function() {
				...
				if(statusRow.indexOf("2") >-1 || statusRow.indexOf("3") >-1){
					...
					$.modal.confirm("确认要拒绝选中的" + rows.length + "条数据吗?", function() {
						var url = $.table._option.refuseUrl;
						var data = { "ids": rows.join() };
						$.operate.submit(url, "post", "json", data);
					});
				}
    	...
    	common: {
			...
			isNotPhone: function(phone) {
				if($.common.isNotEmpty(phone)){
					var phoneExe=/^1[3|4|5|6|7|8|9][0-9]\d{8}$/;
					if(!phoneExe.test(phone)){
						return true;
					}else{
						return false;
					}
			...
			//页面弹框提示是否是正整数
			isNotNum: function(number) {
				if($.common.isNotEmpty(number)){
					var numReg=/^[1-9][0-9]*$/;
		...
		work:{
			...
			workShopTree: function() {
				...
				var options = {
					...
					width: "380",
					url: url,
					callBack: $.work.shopSubmit
				};
				$.modal.openOptions(options);
			},

			shopSubmit: function(index, layero){
				var tree = layero.find("iframe")[0].contentWindow.$._tree;
				if ($.tree.notAllowParents(tree)) {
					var body = layer.getChildFrame('body', index);
					$("#workshopId").val(body.find('#treeId').val());
					$("#workshopName").val(body.find('#treeName').val());
					layer.close(index);
				}
			},

			/*工区-选择部门树*/
			workAreaTree:function() {
				var treeId = $("#treeId").val();
				var deptId = $.common.isEmpty(treeId) ? "1" : $("#treeId").val();
				var url = ctx + "system/dept/selectDeptTree/" + deptId;
				var options = {
					...
					width: "380",
					url: url,
					callBack: $.work.areaSubmit
				};
				$.modal.openOptions(options);
			},

			areaSubmit: function(index, layero){
				var tree = layero.find("iframe")[0].contentWindow.$._tree;
				if ($.tree.notAllowParents(tree)) {
					var body = layer.getChildFrame('body', index);
					$("#workareaId").val(body.find('#treeId').val());
					$("#workareaName").val(body.find('#treeName').val());
					layer.close(index);
				}
			},

			/*工区-选择部门树*/
			workDeptTree:function() {
				var treeId = $("#treeId").val();
				var deptId = $.common.isEmpty(treeId) ? "1" : $("#treeId").val();
				var url = ctx + "system/dept/selectDeptTree/" + deptId;
				var options = {
					...
					width: "380",
					url: url,
					callBack: $.work.deptSubmit
				};
				$.modal.openOptions(options);
			},

			deptSubmit: function(index, layero){
				var tree = layero.find("iframe")[0].contentWindow.$._tree;
				if ($.tree.notAllowParents(tree)) {
					var body = layer.getChildFrame('body', index);
